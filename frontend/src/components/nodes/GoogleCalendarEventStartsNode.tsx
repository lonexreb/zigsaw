...
interface GoogleCalendarEventStartsNodeProps {
  id: string
  data: {
    label: string
    description: string
...
export function GoogleCalendarEventStartsNode({ id, data, selected }: GoogleCalendarEventStartsNodeProps) {
    ...
    <div className={`relative w-[320px] min-h-[120px] overflow-auto scrollbar-hide backdrop-blur-xl border-2 rounded-xl p-4 shadow-lg transition-all duration-300 ${getStatusColor()} ${selected ? 'ring-2 ring-blue-400/50' : ''}`}>
      ...
      <div className="flex items-center space-x-3 mb-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-bold text-blue-300 text-base truncate">When calendar event starts</div>
          <div className="text-xs text-blue-200/70 truncate">{data.description || 'Trigger when a Google Calendar event is about to begin.'}</div>
        </div>
      </div>
    </div>
  )
